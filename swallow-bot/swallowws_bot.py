...
if CONFIG_WAY == "local":
    ...
    config = {'token': token, 'mysql_host': mysql_host, 'mysql_user': mysql_user, 'mysql_passwd': mysql_passwd, 'mysql_db': mysql_db}
...
def get_data_from_database():
    db = MySQLdb.connect(host=config['mysql_host'], user=config['mysql_user'], passwd=config['mysql_passwd'], db=config['mysql_db'])
    cursor = db.cursor()
    try:
        ...
        return {'dateTime'     : (int(dateTime) if dateTime is not None else '---'),
                'pressure'     : (int(pressure * 0.750064) if pressure is not None else '0'),  # hPa to mmHg
                'outTemp'      : (round(float(outTemp), 1) if outTemp is not None else '0'),
                'inTemp'       : (round(float(inTemp), 1) if inTemp is not None else '0'),
                'outHumidity'  : (int(outHumidity) if outHumidity is not None else '0'),
                'windSpeed'    : (round(float(windSpeed), 1) if windSpeed is not None else '0'),
                'deltarain'    : (int(deltarain) if deltarain is not None else '0'),
                'geiger'       : (int(geiger) if geiger is not None else '0'),
                'illumination' : (int(illumination) if illumination is not None else '0')
                }
